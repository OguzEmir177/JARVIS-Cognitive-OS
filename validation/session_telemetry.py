...
==========================================================
Run: python session_telemetry.py
It runs in the background during manual tests.
It takes a snapshot every 30 seconds.
Stop with Ctrl+C → produces session_report.json."""
...
try:
    ...
    HAS_PSUTIL = False
    print("⚠️ no psutil — RAM/CPU monitoring disabled. pip install psutil")
...
async def background_monitor(interval_s: int = 30):
    print(f"📊 Telemetry started — every {interval_s}s snapshot")
    print("Stop and get report with Ctrl+C\n")
...
def save_report():
    ...
    print(f"Duration: {report['session_duration_min']} minutes")

    if "ram_analysis" in report:
        ra = report["ram_analysis"]
        growth_ok = ra["growth_mb"] < 200
        print(f"RAM start: {ra['start_mb']} MB → end: {ra['end_mb']} MB")
        print(f"Growth: {ra['growth_mb']} MB ({'✅ normal' if growth_ok else '⚠️ yüksek'})")
