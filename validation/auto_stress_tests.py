...
<body>
<h1>🤖 J.A.R.V.I.S. V14 — Bilişsel Doğrulama Raporu</h1>
<p style="color: #8ab4f8;">Oluşturulma: {report['timestamp']} | Toplam Süre: {report['total_time_s']}s</p>
<div class="score-card">
  <div>
    <div class="score">{report['overall_score']:.2f}<span style="font-size:.4em; color:#7fc8ff">/1.00</span></div>
    <div class="score-meta">Genel Sistem Kararlılık Skoru</div>
  </div>
  <div style="text-align: right;">
    <div style="font-size: 1.2em; margin-bottom: 8px;">
      <span class="PASS">✓ {report['summary']['passed']} Başarılı</span> | 
      <span class="FAIL">✗ {report['summary']['failed']} Başarısız</span>
    </div>
    <div style="color: #8ab4f8;">
      Hatalar: {report['summary']['errors']} | Atlananlar: {report['summary']['skipped']}
    </div>
  </div>
</div>
<h2>📌 Özet Tablosu</h2>
<table>
<tr><th>Test Modülü</th><th>Durum</th><th>Skor</th><th>Başarı Oranı</th></tr>
"""
  ...
  <td><div class="bar"><div class="bar-fill" style="width:{fill}%"></div></div></td>
    ...
    html += "</table>\n<h2>🔍 Detaylı Analiz & Nokta Atışı Teşhisler</h2>"
    ...
    for key, res in report["tests"].items():
        ...
        diag_html = ""
        if status == "PASS":
            proof = "Sistem bu modülde beklenen tüm sınır koşullarını hatasız geçmiştir."
            if key == "hypothesis_engine": proof = f"Kanıt: {details.get('active_after_prune')} aktif hipotez 50 sınırının altında tutuldu ve doğru bilgi pekiştirildi."
            elif key == "workflow_inference": proof = f"Kanıt: Çapraz uygulama (VSCode -> Chrome vb.) geçişlerinde bağlam kopmadı, {details.get('post_interrupt_workflows')} akış hatasız korundu."
            elif key == "event_storm": proof = f"Kanıt: Saniyede {details.get('events_per_second')} olay (toplam {details.get('events_fired')}) EventBus'a pompalandı, sistem çökmek yerine tümünü izole etti."
            elif key == "memory_growth": proof = f"Kanıt: 200 döngülük uzun kullanım simülasyonunda hafıza büyümesi {details.get('memory_growth_kb')}KB ile güvenli sınırın (5000KB) çok altında kaldı. Sızıntı (Leak) yok."
            elif key == "tool_router": proof = f"Kanıt: {details.get('correct_routes')}/{details.get('total_cases')} karmaşık Türkçe komut kusursuz şekilde doğru protokollere yönlendirildi."
            
            diag_html = f"""<div class="diagnostic success">
              <div class="diag-title success">✅ Mükemmellik Kanıtı (Proof of Perfection)</div>
              <div>{proof} Her şey tam olması gerektiği gibi, stabil ve hatasız çalışıyor.</div>
            </div>"""
        else:
            cause = err if err else "Bilinmeyen bir eşik altı sonuç alındı."
            if key == "hypothesis_engine" and details.get("active_after_prune", 0) > 50: cause = "Kök Neden: Pruning (budama) fonksiyonu çalışmadı, hafıza sızıntısı riski var. 'max_active' limiti aşıldı."
            elif key == "workflow_inference" and not details.get("dev_workflow_survives_interruption"): cause = "Kök Neden: Araya giren başka bir işlem (örn: Spotify açılması) asıl çalışma bağlamını (VSCode) unutturdu. Context overlap threshold çok yüksek olabilir."
            elif key == "memory_growth" and details.get("memory_growth_kb", 0) > 5000: cause = f"Kök Neden: Ciddi Bellek Sızıntısı (Memory Leak)! 200 döngüde {details.get('memory_growth_kb')}KB büyüme var. EnvironmentGraph veya EventBus'ta temizlenmeyen objeler birikiyor."
            elif key == "tool_router": cause = "Kök Neden: 'Aç' vs 'Ara' gibi fiil kökleri semantik yönlendirici tarafından karıştırılıyor. tool_router.py içerisindeki keyword önceliklendirmesi (routing rules) başarısız oldu."
            elif err: cause = f"Kök Neden (Fatal Crash): {err}\nModül çöktüğü için test tamamlanamadı. Stack trace incelenmeli."

            diag_html = f"""<div class="diagnostic danger">
              <div class="diag-title danger">🚨 Nokta Atışı Teşhis (Root Cause Diagnostic)</div>
              <div>{cause} Lütfen ilgili modüldeki bu kök nedene odaklanın.</div>
            </div>"""
  ...
  <details style="margin-top: 15px;">
    <summary style="color: #8ab4f8; cursor: pointer;">Ham Test Verilerini Görüntüle (JSON)</summary>
